...
for (( r=0; r<3; r++)) do
    for mesh in ${Meshes[@]}; do
        for method in ${Methods[@]}; do
            echo "$Runname" -G ../optional/gsUnstructuredSplines/filedata/surfaces/neon/neon_side_split_pillar_"$mesh"_p2.xml -B $BVP -m $method -r $r -p $p -s $s --plot -N 16 -S $S --write -o "$out"ModalResults/Car_pillar_"$mesh"_r"$r"_p"$p"_s"$s"_m"$method"
            eval "$Runname" -G ../optional/gsUnstructuredSplines/filedata/surfaces/neon/neon_side_split_pillar_"$mesh"_p2.xml -B $BVP -m $method -r $r -p $p -s $s --plot -N 16 -S $S --write -o "$out"ModalResults/Car_pillar_"$mesh"_r"$r"_p"$p"_s"$s"_m"$method" 1> "$out"ModalResults/Car_pillar_"$mesh"_r"$r"_p"$p"_s"$s"_m"$method".log 2> "$out"ModalResults/Car_pillar_"$mesh"_r"$r"_p"$p"_s"$s"_m"$method".err
        done
    done
done

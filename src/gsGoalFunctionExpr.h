...
public:
    ...
    void setAssembler(gsExprAssembler<T> * assembler) { m_assembler = assembler;}
...
public:
    gsGoalFunction( const gsMultiPatch<T> & original,
...
public:
    ...
    gsGoalFunction( const gsMultiPatch<T> & original,
                    const gsMultiPatch<T> & deformed,
                    const gsMaterialMatrixBase<T> * materialMatrix)
    :
    m_ori(original),
    m_def(deformed)
    {
        m_materialMatrices = gsMaterialMatrixContainer<T>(original.nPatches());
        ...
        for (size_t p=0; p!=original.nPatches(); p++)

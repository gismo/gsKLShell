...
gsMaterialMatrix<dim,T,mat,comp>::gsMaterialMatrix(
...
// Composite material model
template <short_t dim, class T, short_t mat, bool comp>
gsMaterialMatrix<dim,T,mat,comp>::gsMaterialMatrix(  const gsFunctionSet<T>            & mp,
                                        const gsFunctionSet<T>            & mp_def,
                                        const std::vector<T>                thickness,
                                        const std::vector<std::pair<T,T>> & YoungsModuli,
                                        const std::vector<T>              & ShearModuli,
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::info() const
{
    gsInfo  <<"---------------------------------------------------------------------\n"
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::defaultOptions()
{
...
template <short_t dim, class T, short_t mat, bool comp>
short_t gsMaterialMatrix<dim,T,mat,comp>::domainDim() const { return 2; }

template <short_t dim, class T, short_t mat, bool comp>
short_t gsMaterialMatrix<dim,T,mat,comp>::targetDim() const
{
    if (m_outputType==2)
        return 9;
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::initialize()
{
    // Set default options
    this->defaultOptions();
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::computePoints(const gsMatrix<T> & u, bool deformed) const
{
    gsMatrix<T> tmp;
...
typename std::enable_if<_mat==14, void>::type
gsMaterialMatrix<dim,T,mat,comp>::computePoints_impl(const gsMatrix<T>& u, bool deformed) const
{
    T prod, sum, mu;
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::eval_into(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
    this->getOptions();
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::eval_into_dens(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::eval_into_stretch(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
    m_map.points = u;
    static_cast<const gsFunction<T>&>(m_patches->piece(0)   ).computeMap(m_map); // the piece(0) here implies that if you call class.eval_into, it will be evaluated on piece(0). Hence, call class.piece(k).eval_into()
    ...
    eval_into_stretch_impl<comp>(u,result);
}

template <short_t dim, class T, short_t mat, bool comp>
template<bool _comp> inline
typename std::enable_if<!_comp, void>::type
gsMaterialMatrix<dim,T,mat,comp>::eval_into_stretch_impl(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
    result.resize(this->targetDim(), u.cols());
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::eval_into_stretchdir(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
    m_map.points = u;
    static_cast<const gsFunction<T>&>(m_patches->piece(0)   ).computeMap(m_map); // the piece(0) here implies that if you call class.eval_into, it will be evaluated on piece(0). Hence, call class.piece(k).eval_into()
    ...
    eval_into_stretchdir_impl<comp>(u,result);
}

template <short_t dim, class T, short_t mat, bool comp>
template<bool _comp> inline
typename std::enable_if<!_comp, void>::type
gsMaterialMatrix<dim,T,mat,comp>::eval_into_stretchdir_impl(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
    result.resize(this->targetDim(), u.cols());
    std::pair<gsVector<T>,gsMatrix<T>> res;
    for (index_t i=0; i!= u.cols(); i++)
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::eval_into_NP(const gsMatrix<T>& u, gsMatrix<T>& result) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
gsMatrix<T> gsMaterialMatrix<dim,T,mat,comp>::eval3D(const index_t i, const gsMatrix<T> & z) const
{
    return eval3D_impl<mat,comp>(i,z);
}

template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat, bool _comp> inline
typename std::enable_if<_mat==0, gsMatrix<T>>::type
gsMaterialMatrix<dim,T,mat,comp>::eval3D_impl(const index_t i, const gsMatrix<T>& z) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat, bool _comp> inline
typename std::enable_if<!_comp && !(_mat==0), gsMatrix<T>>::type
gsMaterialMatrix<dim,T,mat,comp>::eval3D_impl(const index_t i, const gsMatrix<T>& z) const
{
    gsMatrix<T> result = eval_Incompressible(i, z);
    return result;
...
template <short_t dim, class T, short_t mat, bool comp>
gsMatrix<T> gsMaterialMatrix<dim,T,mat,comp>::eval3D(const index_t i) const
{
    gsMatrix<T> z(1,1);
    z.setZero();
    return eval3D(i,z);
...
template <short_t dim, class T, short_t mat, bool comp>
gsMatrix<T> gsMaterialMatrix<dim,T,mat,comp>::multiplyZ(const gsMatrix<T>& u) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
gsMatrix<T> gsMaterialMatrix<dim,T,mat,comp>::integrateZ(const gsMatrix<T>& u) const
{
    // Input: points in R2
    // Ouput: results in targetDim
...
template <short_t dim, class T, short_t mat, bool comp>
gsMatrix<T> gsMaterialMatrix<dim,T,mat,comp>::eval_Incompressible(const index_t i, const gsMatrix<T>& z) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==0, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl_impl(const index_t i, const index_t j, const index_t k, const index_t l) const
{
    // --------------------------
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==2, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl_impl(const index_t i, const index_t j, const index_t k, const index_t l) const
{
    // --------------------------
    // Neo-Hookean
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==3, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl_impl(const index_t i, const index_t j, const index_t k, const index_t l) const
{
    // --------------------------
    // Mooney-Rivlin
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<(_mat >= 10) && (_mat < 20), T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl_impl(const index_t i, const index_t j, const index_t k, const index_t l) const
{
    // --------------------------
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<(_mat >= 20) && (_mat < 30), T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl_impl(const index_t i, const index_t j, const index_t k, const index_t l) const
{
    // --------------------------
...
// gsMaterialMatrix<dim,T,mat,comp>::Cijkl_impl(const index_t i, const index_t j, const index_t k, const index_t l) const
// {
//     GISMO_ERROR("Material model unknown (model = "<<_mat<<"). Use gsMaterialMatrix<dim,T,mat,comp>::info() to see the options.");
// }
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::Cijkl(const index_t i, const index_t j, const index_t k, const index_t l, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
...
template<short_t _mat> inline
typename std::enable_if<(_mat >= 10) && (_mat < 20), T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl_impl(const index_t i, const index_t j, const index_t k, const index_t l, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::Cijkl3D(const index_t i, const index_t j, const index_t k, const index_t l, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
    GISMO_ASSERT( ( (i <3) && (j <3) && (k <3) && (l <3) ) , "Index out of range. i="<<i<<", j="<<j<<", k="<<k<<", l="<<l);
    return Cijkl3D_impl<mat>(i,j,k,l,c,cinv);
}

template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==0, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl3D_impl(const index_t i, const index_t j, const index_t k, const index_t l, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==5, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cijkl3D_impl(const index_t i, const index_t j, const index_t k, const index_t l, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
    // --------------------------
    // Neo-Hookean 2
...
*/
// template <short_t dim, class T, short_t mat, bool comp>
// T gsMaterialMatrix<dim,T,mat,comp>::Sij(const index_t i, const index_t j) const { Sij(i,j,NULL,NULL); }

template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::Sij(const index_t i, const index_t j) const
{
    return Sij_impl<mat>(i,j);
}

template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==0, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Sij_impl(const index_t i, const index_t j) const
{
    gsMatrix<T> stress;
    // --------------------------
    // Saint Venant Kirchhoff
...
template<short_t _mat> inline
typename std::enable_if<_mat==2, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Sij_impl(const index_t i, const index_t j, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
    // --------------------------
    // Neo-Hoookean
    // --------------------------
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::Sii(const index_t i) const // principle stresses
{
    gsMatrix<T> C(3,3);
    C.setZero();
    C.block(0,0,2,2) = m_Gcov_def.block(0,0,2,2);
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::Sii(const index_t i, const gsMatrix<T> & c) const
{
    computeStretch(c);
    return Sa(i);
}
...
template <short_t dim, class T, short_t mat, bool comp>
gsMatrix<T> gsMaterialMatrix<dim,T,mat,comp>::eval_Compressible(const index_t i, const gsMatrix<T>& z) const
{
    // Input: j index in-plane point
    //        z out-of-plane coordinate (through thickness) in R1 (z)
    gsMatrix<T> result(this->targetDim(), z.cols());
        ...
        for (index_t it = 0; it < itmax; it++)
        {
            ...
            if (abs(dc33) < tol)
            {
                ...
                if (m_outputType==2)
                    ...
                    {
                        GISMO_ASSERT(mat >= 10 && mat < 20, "Only available for stretch-based materials.");
...
gsMaterialMatrix<dim,T,mat,comp>::dPsi_impl(const index_t i, const index_t j) const
{
    T mu = m_parvals.at(0) / (2. * (1. + m_parvals.at(1)));
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==23, T>::type
gsMaterialMatrix<dim,T,mat,comp>::dPsi_impl(const index_t i, const index_t j) const
{
    T mu = m_parvals.at(0) / (2. * (1. + m_parvals.at(1)));
    T c2 = mu/(m_parvals.at(2)+1);
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::dI_1(const index_t i, const index_t j) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::dI_2(const index_t i, const index_t j, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
    T traceCt = m_Gcov_def(0,0)*m_Gcon_ori(0,0) +
                m_Gcov_def(0,1)*m_Gcon_ori(0,1) +
                m_Gcov_def(1,0)*m_Gcon_ori(1,0) +
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::dPsi(const index_t i, const index_t j, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
...
template<short_t _mat> inline
typename std::enable_if<_mat==22, T>::type
gsMaterialMatrix<dim,T,mat,comp>::dPsi_impl(const index_t i, const index_t j, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
    GISMO_ASSERT(3 - 6 * m_parvals.at(1) != 0, "Bulk modulus is infinity for compressible material model. Try to use incompressible models.");
    T mu = m_parvals.at(0) / (2. * (1. + m_parvals.at(1)));
    T K  = m_parvals.at(0) / ( 3 - 6 * m_parvals.at(1));
    ...
    T traceCt = m_Gcov_def(0,0)*m_Gcon_ori(0,0) +
                ...
                m_Gcov_def(1,1)*m_Gcon_ori(1,1);
    T I_1   = traceCt + c(2,2);
    return mu/2.0 * math::pow(m_J_sq,-1./3.) * ( - 1.0/3.0 * I_1 * cinv(i,j) + dI_1(i,j) ) + dpsi_vol;
}
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat> inline
typename std::enable_if<_mat==25, T>::type
gsMaterialMatrix<dim,T,mat,comp>::dPsi_impl(const index_t i, const index_t j, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
    T mu = m_parvals.at(0) / (2. * (1. + m_parvals.at(1)));
    T lambda = m_parvals.at(0) * m_parvals.at(1) / ( (1. + m_parvals.at(1))*(1.-2.*m_parvals.at(1)));
...
template<short_t _mat> inline
typename std::enable_if<_mat==22, T>::type
gsMaterialMatrix<dim,T,mat,comp>::d2Psi_impl(const index_t i, const index_t j, const index_t k, const index_t l, const gsMatrix<T> & c, const gsMatrix<T> & cinv) const
{
    GISMO_ASSERT(3 - 6 * m_parvals.at(1) != 0, "Bulk modulus is infinity for compressible material model. Try to use incompressible models.");
    T mu = m_parvals.at(0) / (2. * (1. + m_parvals.at(1)));
    T dCinv = - 1./2.*( cinv(i,k)*cinv(j,l) + cinv(i,l)*cinv(j,k) );
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::dPsi_da(const index_t a) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat, bool _comp> inline
typename std::enable_if<_comp && (_mat==2 || _mat==12 || _mat==22), T>::type
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat, bool _comp> inline
typename std::enable_if<_comp && (_mat==3 || _mat==13 || _mat==23), T>::type
gsMaterialMatrix<dim,T,mat,comp>::dPsi_da_impl(const index_t a) const
{
    T mu = m_parvals.at(0) / (2 * (1 + m_parvals.at(1)));
    T I_1   = m_stretches(0)*m_stretches(0) + m_stretches(1)*m_stretches(1) + m_stretches(2)*m_stretches(2);
...
template <short_t dim, class T, short_t mat, bool comp>
template<short_t _mat, bool _comp> inline
typename std::enable_if<_comp && _mat==14, T>::type
gsMaterialMatrix<dim,T,mat,comp>::dPsi_da_impl(const index_t a) const
{
    GISMO_ASSERT(3 - 6 * m_parvals.at(1) != 0, "Bulk modulus is infinity for compressible material model. Try to use incompressible models.");
    T beta  = -2.0;T K  = m_parvals.at(0) / ( 3 - 6 * m_parvals.at(1));
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::d2Psi_dab(const index_t a, const index_t b) const
{
...
template<short_t _mat, bool _comp> inline
typename std::enable_if<_comp && (_mat==2 || _mat==12 || _mat==22), T>::type
gsMaterialMatrix<dim,T,mat,comp>::d2Psi_dab_impl(const index_t a, const index_t b) const
{
    T mu = m_parvals.at(0) / (2 * (1 + m_parvals.at(1)));
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::dJ_da(const index_t a) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::d2J_dab(const index_t a, const index_t b) const
{
    if (a==b)
        return 0.0;
    else
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::p() const
{
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::dp_da(const index_t a) const
{
    if (a==2)
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::Sa(const index_t a) const
{
...
gsMaterialMatrix<dim,T,mat,comp>::Sa_impl(const index_t a) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::dSa_db(const index_t a, const index_t b) const
{
...
template<bool _comp> inline
typename std::enable_if<_comp, T>::type
gsMaterialMatrix<dim,T,mat,comp>::dSa_db_impl(const index_t a, const index_t b) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
T gsMaterialMatrix<dim,T,mat,comp>::Cabcd(const index_t a, const index_t b, const index_t c, const index_t d) const
{
...
template<bool _comp> inline
typename std::enable_if<_comp, T>::type
gsMaterialMatrix<dim,T,mat,comp>::Cabcd_impl(const index_t a, const index_t b, const index_t c, const index_t d) const
{
...
// template <short_t dim, class T, short_t mat, bool comp>
// T gsMaterialMatrix<dim,T,mat,comp>::dPsi(const index_t a) const
// {
//     T mu = m_parvals.at(0) / (2. * (1. + m_parvals.at(1)));
...
// template <short_t dim, class T, short_t mat, bool comp>
// T gsMaterialMatrix<dim,T,mat,comp>::d2Psi(const index_t a, const index_t b) const
// {
//     T mu = m_parvals.at(0) / (2. * (1. + m_parvals.at(1)));
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::computeMetricDeformed() const
{
...
gsMaterialMatrix<dim,T,mat,comp>::computeMetricDeformed_impl() const
{
    gsMatrix<T> deriv2;
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::computeMetricUndeformed() const
{
...
template <short_t _dim> inline
typename std::enable_if<_dim==3, void>::type
gsMaterialMatrix<dim,T,mat,comp>::computeMetricUndeformed_impl() const
{
    gsMatrix<T> deriv2;
    gsMatrix<T,3,1> normal;
    gsMatrix<T,2,2> mixedB;
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::getMetric(index_t k, T z) const
{
...
template <short_t _dim> inline
typename std::enable_if<_dim==3, void>::type
gsMaterialMatrix<dim,T,mat,comp>::getMetricDeformed_impl(index_t k, T z) const
{
...
template <short_t _dim> inline
typename std::enable_if<_dim==3, void>::type
gsMaterialMatrix<dim,T,mat,comp>::getMetricUndeformed_impl(index_t k, T z) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
std::pair<gsVector<T>,gsMatrix<T>> gsMaterialMatrix<dim,T,mat,comp>::evalStretch(const gsMatrix<T> & C) const
{
...
template <short_t dim, class T, short_t mat, bool comp>
void gsMaterialMatrix<dim,T,mat,comp>::computeStretch(const gsMatrix<T> & C) const
{

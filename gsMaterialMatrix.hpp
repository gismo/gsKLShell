...
// template <short_t dim, class T, index_t matId, bool comp, enum Material mat, enum Implementation imp >
// void gsMaterialMatrix<dim,T,matId,comp,mat,imp>::_computeMetricUndeformed(const index_t patch, const gsMatrix<T> & u) const
// {
//     m_map.flags = NEED_JACOBIAN | NEED_DERIV | NEED_NORMAL | NEED_VALUE | NEED_DERIV2;
//     m_map.points = u;
//     static_cast<const gsFunction<T>&>(m_patches->piece(patch)   ).computeMap(m_map); // the piece(0) here implies that if you call class.eval_into, it will be evaluated on piece(0). Hence, call class.piece(k).eval_into()
...
// gsMaterialMatrix<dim,T,matId,comp,mat,imp>::_computeMetricUndeformed_impl(const index_t patch, const gsMatrix<T> & u) const
// {
//     gsMatrix<T> deriv2;
...
// template <short_t dim, class T, index_t matId, bool comp, enum Material mat, enum Implementation imp >
// std::pair<gsVector<T>,gsMatrix<T>> gsMaterialMatrix<dim,T,matId,comp,mat,imp>::this->_evalStretch(const gsMatrix<T> & C) const
// {
...
// template <short_t dim, class T, index_t matId, bool comp, enum Material mat, enum Implementation imp >
// void gsMaterialMatrix<dim,T,matId,comp,mat,imp>::this->_computeStretch(const gsMatrix<T> & C) const
// {
//     std::pair<gsVector<T>,gsMatrix<T>> result = this->_evalStretch(C);

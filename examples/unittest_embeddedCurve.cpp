...
int main(int argc, char *argv[])
{
    ...
    gsMatrix<real_t> coef_c(basis_c.size(), surf-> parDim()); //fill stiffener control net
    coef_c.col(0) = cpvec;
    coef_c.col(1) = cpvec_flipped;
    gsBSpline<> stiffener1(basis_c, coef_c);
    ...
    gsDebug<<exprEvaluator.eval( gismo::expr::cnv_vara_normalized(G_surf, G_curve), gamma ).transpose() << "\n";
    ...
    // gsDebug<<exprEvaluator.eval( gismo::expr::cnv_vara_var1(u_surf,G_surf,G_curve), gamma) << "\n";
    // gsDebug<<exprEvaluator.eval( gismo::expr::cnv_vara_var1_normalized(u_surf,G_surf,G_curve), gamma) << "\n";
    ...
    // gsDebug<<exprEvaluator.eval( gismo::expr::cbv_vara_var1(u_surf,G_surf,G_curve), gamma) << "\n";
    // gsDebug<<exprEvaluator.eval( gismo::expr::cbv_vara_var1_normalized(u_surf,G_surf,G_curve), gamma) << "\n";
    ...
    /*
        The following functions are provided:
            ...
            k21_der     first variation of k21.  
    ...
    **/
    ...
    auto k21_der = cnv_vara_var1_normalized(u_surf,defG_surf,G_curve)*ctv(defG_surf, G_curve) + ctv_var1(u_surf,defG_surf,G_curve)*cnv_vara(defG_surf, G_curve).normalized();
    auto k21_der2 = cnv_vara_var2dot(u_surf,u_surf,G_surf,G_curve,ctv(defG_surf, G_curve)) +
                    2*var1_dot_var1(u_surf,u_surf,G_curve,cnv_vara_var1_normalized(u_surf,defG_surf,G_curve),ctv_var1(u_surf,defG_surf,G_curve));      
    ...
    auto k31_der = cbv_vara_var1_normalized(u_surf,defG_surf,G_curve)*ctv(defG_surf, G_curve) + ctv_var1(u_surf,defG_surf,G_curve)*cbv_vara(defG_surf, G_curve).normalized();
    auto k31_der2 = cbv_vara_var2dot(u_surf,u_surf,defG_surf,G_curve,ctv(defG_surf, G_curve)) +
                    2*var1_dot_var1(u_surf,u_surf,G_curve,cbv_vara_var1_normalized(u_surf,defG_surf,G_curve),ctv_var1(u_surf,defG_surf,G_curve));
    ...
    auto k23_der = cnv_vara_var1_normalized(u_surf,defG_surf,G_curve)*cbv(defG_surf, G_curve) + cbv_var1(u_surf,defG_surf,G_curve)*cnv_vara(defG_surf, G_curve).normalized();
    auto k23_der2 = cnv_vara_var2dot(u_surf,u_surf,G_surf,G_curve,cbv(defG_surf, G_curve)) +
                    2*var1_dot_var1(u_surf,u_surf,G_curve,cnv_vara_var1_normalized(u_surf,defG_surf,G_curve),cbv_var1(u_surf,defG_surf,G_curve)) +
                    cbv_var2dot(u_surf,u_surf,defG_surf,G_curve,cnv_vara(defG_surf,G_curve).normalized());
    ...
    auto k32_der = cbv_vara_var1_normalized(u_surf,defG_surf,G_curve)*cnv(defG_surf, G_curve) + cnv_var1(u_surf,defG_surf,G_curve)*cbv_vara(defG_surf, G_curve).normalized();
    auto k32_der2 = cbv_vara_var2dot(u_surf,u_surf,defG_surf,G_curve,cnv(defG_surf,G_curve)) +
                    2*var1_dot_var1(u_surf,u_surf,G_curve,cbv_vara_var1_normalized(u_surf,defG_surf,G_curve),cnv_var1(u_surf,defG_surf,G_curve)) +
                    cnv_var2dot(u_surf,u_surf,G_curve,cbv_vara(defG_surf,G_curve).normalized());

    // integrands in beam stiffness matrix
    auto KintBendingAxial_b = E_modulus_b/pow(ctv(G_surf, G_curve).norm(),3) * 
    ...
    auto KintTorsion_b = G_modulus_b*inertiap_b/(2*pow(ctv(G_surf, G_curve).norm(),3)) *
                         (-k32_der * k32_der.tr() + k23_der * k23_der.tr() + k23.val() * k23_der2 + (-k32.val()) * k32_der2);

    // integrands in beam residual force vector
    auto RintBendingAxial_b = E_modulus_b/pow(ctv(G_surf, G_curve).norm(),3) * 
                         (area_b * eps.val() * eps_der + inertiamax_b * k21.val() * k21_der + inertiamin_b * k31.val() * k31_der);
    auto RintTorsion_b = G_modulus_b*inertiap_b/(2*pow(ctv(G_surf, G_curve).norm(),3)) *
                         (-k32.val() * k32_der + k23.val() * k23_der);

    // gsDebugVar(exprEvaluator.eval(cnv_vara_var2dot(u_surf,u_surf,G_surf,G_curve,ctv(defG_surf, G_curve)),gamma));
    //gsDebugVar(exprEvaluator.eval(massmat(u_surf,u_surf,G_curve,0.00785),gamma));
